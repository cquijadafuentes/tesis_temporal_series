...
//    cout << "Leyendo series temporales..." << endl;
    ...
    vector<vector<double>> stPromedio(rows, vector<double>(cols));

    double promedioStPromedio = 0.0;
    for(int f=0; f<rows; f++){
        for(int c=0; c<cols; c++){
            stPromedio[f][c] = promedioST(temporalSeries[f][c]);
            promedioStPromedio += stPromedio[f][c];
        }
    }
    promedioStPromedio /= (rows*cols);
//    cout << "Promedio: " << promedioStPromedio << endl;
...
//    cout << "Calculando I de Moran.. " << endl;
    for(int f1 = 0; f1 < rows; f1++){
        for(int c1 = 0; c1 < cols; c1++) {
            double diff_i_promedio = stPromedio[f1][c1] - promedioStPromedio;
//            cout << "Desde celda: " << print_celda(f1, c1, stPromedio[f1][c1]) << " - " << diff_i_promedio << endl;
...
//                        cout << "\tRevisando celda: " << print_celda(f2, c2, stPromedio[f2][c2]);
                        ...
                        w = 1.0;
                        sumaW += w;
                        aux = w * (diff_i_promedio) * (stPromedio[f2][c2] - promedioStPromedio);
...
//    cout << "denominador: " << denominador << endl;
    ...
    cout << argv[1] << " Moran's I (ro1 promedios)= " << moran_I << endl;
    return 0;
}
